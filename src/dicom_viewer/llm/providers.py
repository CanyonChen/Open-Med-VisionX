...
class _BaseProvider(LLMProvider):
    ...
    def authorize_image_transfer(self) -> int:
        """Grant provider-local consent for future rendered-preview requests."""

        return self._image_authorization.grant()
    ...
    def chat(
        ...
        *,
        ...
        normalized = self._validate_input(messages, preview)
        ...
        if preview is not None:
            ...
            if not self._image_authorization.authorized:
                raise CloudTransferDenied(
                    "Cloud image transfer is disabled. "
                    "The user must explicitly authorize this provider."
                )
        return normalized

    def _headers(self, *, stream: bool) -> dict[str, str]:
        api_key = self._credentials.resolve(self._config.credential_ref)
        headers = self._authentication_headers(api_key)
