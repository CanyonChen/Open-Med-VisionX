...
class CancellationToken:
    ...
    def _try_cancel(self) -> bool:
        """Atomically request cancellation unless an irreversible commit began."""
    ...
    def _try_seal(self) -> bool:
        """Close the cancellation window if no request has already won the race."""
...
class TaskContext:
    ...
    def enter_commit_phase(self) -> None:
        """Atomically enter an irreversible externally visible commit.

        Call this immediately before the commit.  An earlier cancellation
        request wins and raises :class:`OperationCancelled`; after this method
        returns, future :meth:`BackgroundTask.cancel` calls are too late and
        return ``False``.  The boundary is deliberately one-way, including
        when the subsequent commit itself fails.
        """

        if not self._token._try_seal():
            raise OperationCancelled("Background operation was cancelled before its commit phase.")
...
class BackgroundTask(Generic[T]):
    ...
    def cancel(self) -> bool:
        ...
        Returns ``False`` when the task has already reached a terminal state
        or crossed its irreversible commit boundary.  Pending work is
        cancelled immediately; running work observes the request through its
        :class:`CancellationToken`.
        """
        ...
        with self._condition:
            if self._status in _TERMINAL_STATES:
                return False
            if not self._token._try_cancel():
                return False
            future = self._future
