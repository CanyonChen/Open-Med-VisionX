...
class CredentialReference:
    ...
    def __post_init__(self) -> None:
        ...
        if scheme not in {"env", "keyring", "none"}:
            raise ValidationError("Credential references must use env:, keyring:, or none.")
        if scheme != "none" and not target:
            raise ValidationError("Credential reference target cannot be empty.")
        if scheme == "none" and target:
            raise ValidationError("The none credential reference cannot have a target.")
    ...
    def parse(cls, value: str | CredentialReference) -> CredentialReference:
        ...
        if not isinstance(value, str) or ":" not in value:
            raise ValidationError(
                "Store a reference such as env:OPENAI_API_KEY or use none for a loopback service; "
                "never enter a raw key."
            )
...
class CredentialResolver:
    """Resolve ``env:``, optional ``keyring:``, or loopback-only ``none`` references.
